...
class ColorParser{
	...
	function __construct($autoContrastToBackgroundColor = ''){
	...
	function drawStyledString($src_img, $size, $x, $y , $color, $font, $text, $stripColors = false, $stripTags = '') {
		...
		{
			$fontUsed = "./".$font;
			if (isset($chunks[$i]['wide'])) $fontUsed .= 'wd';
			if (isset($chunks[$i]['italic'])) $fontUsed .= 'it';
			if (isset($chunks[$i]['caps'])) $chunks[$i]['text'] = strtoupper($chunks[$i]['text']);
			$fontUsed.=".ttf";
